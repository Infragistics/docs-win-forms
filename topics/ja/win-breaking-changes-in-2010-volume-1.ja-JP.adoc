...
= 2010 Volume 1 の重大な変更

== クライアント専用 .NET Framework のサポート

$$.NET$$ Framework のすべてのクライアント専用版をサポートするために、System.Design および System.Drawing.Design への参照がすべての Infragistics アセンブリから削除されています。 すべてのデザイナー、CodeDomSerializer、タイプコンバーター、CollectionEditor から派生する UI タイプ エディターは Infragistics.Win.Design アセンブリに移動されました。
...
|これらのパラメーター (IDesignerSerializationManager, object) を取得するために (IDesignerSerializationManager, CodeDomSerializer) のパラメーターを取得するコンストラクターを変更しました。

|Infragistics.Shared.Serialization.CodeDomSerializationInfo
|CodeDomSerializer からオブジェクトに Serializer プロパティの戻しタイプを変更します。

|Infragistics.Win.Ink.UltraInkProvider
| 

{ApiPlatform}Win.UltraWinInkProvider.v10.1 で定義された UltraInkProvider はスマートタグをサポートしなくなります。 {ApiPlatform}Win.UltraWinInkProvider.Ink17.v10.1 で定義された UltraInkProvider のみがスマートタグをサポートします。

|すべてのタイプ
|クライアント専用フレームワークをインストールしてランタイム アプリケーションでコントロールに添付されたプロパティ グリッドを使用すると、コレクション エディターを使用するプロパティは機能しません。

|====

また同様の機能をサポートするために、Infragistics.Documents.Reports.Report.PathResolver パブリック クラスが Infragistics.Documents.Reports アセンブリから削除されています。

Infragistics.Win.Design アセンブリに移動したタイプ名のリストは、このトピックの<<TypeNames,「変更されたタイプ名:」>>のセクションを参照してください。

== Infragistics.Win.UltraWinGrid.DataAreaUIElement オブジェクトでの変更

Infragistics.Win.UltraWinGrid.DataAreaUIElement のコンストラクターが変更されました。 新しいコンストラクターはタイプ Infragistics.Win.UltraWinGrid.UltraGridLayout の追加パラメーターを必要とします。
