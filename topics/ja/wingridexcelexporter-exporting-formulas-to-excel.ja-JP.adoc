...
[start=1]
. 少なくともひとつのバインドされていない計算された列を持つ WinGrid が必要です。これを実行する方法の詳細は、 link:wincalcmanager-creating-a-calculated-column-in-wingrid.html[WinGrid での計算された列の作成] を参照してください。
[start=2]
...
image::Images/Formula_In_Excel.png[Excelへ数式をエクスポート中]
