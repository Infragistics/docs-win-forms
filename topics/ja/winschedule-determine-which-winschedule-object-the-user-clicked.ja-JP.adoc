...
|metadata|
{
    ...
    "tags": [],
    "guid": "{96BD1935-98D1-4742-B2BD-F350B98FE0D5}","buildFlags": [],
...
= ユーザがクリックした WinSchedule オブジェクトを判別

このトピックはすべての WinSchedule コントロールに適用されます。

マウスがどのタイプのオブジェクトの上にあるかは容易に判断できます。次のサブルーチンは、いくつかの GetFromPoint メソッドを使用して、マウスがどこで右クリックされたかを検出する方法を示しています。

.注:
[NOTE]
====
本トピックは、フォームに WinMonthViewSingle を追加して、pick:[win-forms="link:{ApiPlatform}win.ultrawinschedule{ApiVersion}~infragistics.win.ultrawinschedule.ultraschedulecontrolbase~calendarinfo.html[CalendarInfo]"]  プロパティを WinCalendarInfo™ インスタンスに設定していることを前提にします。
====
