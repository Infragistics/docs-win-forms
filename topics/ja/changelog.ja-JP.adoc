...
|UltraTextEditor で、ContextMenu および ContextMenuStrip が無効になっている場合でも、フォーカスがない状態で端を右クリックするとコンテキスト メニューが表示される

|`3590`

|AppStyling

|Office2010Blue の .isl を使用した場合、Visual Studio 2022 と Visual Studio 2026 でボタンのスタイルが異なる

|`3605`

|ToolbarsManager

|メイン メニュー バー タイプのツールバーを追加できない

|`3632`

|NumericEditor

|SpinIncrement が設定されている状態でキーボードで増減すると、UltraNumericEditor が ValueChanged を2回発生させる
...
BinaryFormatter を新しいカスタムの Infragistics Serializer に置き換えました。これがシリアル化の既定のシリアライザーになります。Microsoft Binary Formatter が有効で、バイナリ データが以前の形式の場合は、逆シリアル化に引き続き Microsoft Binary Formatter が使用されます。

[[BC_26_1]]
==== 重大な変更

新しいカスタム シリアライザーは、Ultimate UI for Windows Forms ツールセットの以前のバージョンで生成されたバイナリ ファイルに対して前方互換性があります。BinaryFormatter で生成された元のファイルは、新しいシリアライザーを使用して引き続き読み取りおよび逆シリアル化できます。ただし、以前のバージョンで使用されていた BinaryFormatter は、新しいバージョンで生成されたファイルを逆シリアル化する機能を持っていません。

=== 新しいチャート機能

==== 範囲棒シリーズ

`RangeBarSeries` が `{DataChartName}` に追加され、`CategoryYAxis` と `NumericXAxis` を組み合わせて、安値から高値までの範囲を横棒として描画できるようになりました。詳細は、link:datachart-category-range-bar-series.html[範囲棒シリーズ] を参照してください。
...
|`3174`
|Tree
|UltraTree が .NET Core で Infragistics.Win.Design DLL を検索する。

|`3316`
|ToolbarsManager
|継承されたフォームを使用している場合、新しい UltraTab を追加しても UltraTab オブジェクトがシリアル化されない。

|`3353`
|Schedule
|UltraWinSchedule - 2 月が 28 日の場合に 4 週目の平日の予定が生成されない。

|`3477`
|Grid
|UltraGrid が .NET Framework 4.8 で AccessViolationException によりクラッシュする。

|`25156`
|Grid
|UltraFormattedTextEditor が Word へのエクスポート時にフォント サイズを反映しない。

|`27128`
|Grid
|UltraGrid のサマリー行にフォーカスを設定できないため、Narrator などのスクリーン リーダーで読み上げることができない。

|`28213`
|ExplorerBar
|[高 DPI] - 画像とフォントのスケーリングが非常に不整合かつ不正確。

|`29665`
|Combo
|[アクセシビリティ][スクリーン リーダー] - チェックボックス オプションがある場合に、UltraCombo がスクリーン リーダーで読み上げられない。

|`32595`
|Grid
|画像を含む高 DPI 環境で、画像が正しくスケーリングされない。

|`32761`
|Editors
|UltraComboEditor でナレーターが選択値を読み上げない。

|`33659`
|DataChart
|Windows Forms で LegendHighlightingMode プロパティが機能しない。

|`34335`
|Grid
|Windows ナレーター - UltraGrid の子行バンドを読み上げる際に余分なテキストが追加される。

|`39557`
|Combo
|UltraCombo を使用すると、アクセシビリティ テキストに余分な「Pane」という単語が追加される。

|`40060`
|GridExcelExporter
|GridExcelExporter で MaxExcelRowCount のデータを使用すると InvalidOperationException エラーが発生する。

|`40087`
|Map
|UltraGeographicMap - ズーム中にタイル間の隙間が表示される。

|`41075`
|ToolbarsManager
|net10 の UltraToolbarsManager で ArgumentOutOfRangeException が発生する。

|`41102`
|ToolbarsManager
|ToolMenuItem で NullReferenceException が発生する。

|`41739`
|GanttView
|GanttView - フォームが閉じられた後に TimelineVisibleDateTimeRangeChanged イベントが発生する。
...
| `2378`
| Editors
| UltraDateTimeEditor において、年および月をそれぞれ変更する際に、常に最後の月と日付が選択される。

| `27870`
| UltraToolbarsManager
| UIAutomationForCodedUITestingEnabled を false に設定すると、コンテキスト メニューが UI オートメーションのアプリケーション ツリーに表示されない。

| `35085`
| UltraGrid
| グループ化でソート済み列を追加する前にサマリー列を設定すると、グリッドが例外をスローする。

| `35658`
| UltraCombo
| UIAutomationForCodedUITestingEnabled を false に設定すると、UltraCombo のドロップダウン アイテムにアクセスできない。

| `36959`
| UltraDataSource
| UltraDataSource デザイナーでキャンセルすると、Form1.cs (デザイン) のレイアウトが消える。

| `38668`
| DataTooltipLayer
| チャートの TitleTextColor を使用すると、TitleTextColor が上書きされる。

| `40238`
| Excel
| Excel 数式パーサーを修正 - Workbook.Load() が Excel.FormulaParseException をスローする問題を修正。

| `40705`
| Editors
| CheckEditor で GlyphInfo を使用したカスタム スタイルを作成できない。

| `41167`
| Excel
| オブジェクトの数式がラウンド トリップされない - カメラ ツールのラウンド トリップのための Excel サポートを追加。

| `41419`
| Excel
| VBA 署名済み Excel ファイルを保存しても、署名/証明書が保持されない。

| `41594`
| UltraDataChart
| AssigningCategoryStyle の args.GetItems が null であるか、フラグメント シリーズのアイテムを更新するために機能しない。

| `41731`
| UltraToolbarsManager
| UltraToolbarsManager の ApplicationMenu が WinForms デザイナーに表示されない。

| `41739`
| UltraGanttView
| フォームが閉じられた後に TimelineVisibleDateTimeRangeChanged イベントが発生する。
|====
...
- `DataPieChart` に `OthersCategoryBrush` と `OthersCategoryOutline` が追加されました。
