...
When the user fixes a column via the UI, the
...
events fire. Likewise, if the user fixes an
...
To listen for this interaction, you can trap for when the link:{ApiPlatform}win.ultrawingrid{ApiVersion}~infragistics.win.ultrawingrid.beforecolposchangedeventargs~ColumnPosChangedType.html[ColumnPosChangedType]
is link:{ApiPlatform}win.ultrawingrid{ApiVersion}~infragistics.win.ultrawingrid.poschangedtype.html[PosChangedType].FixedStateChanged in the event arguments, as demonstrated in the following code:
