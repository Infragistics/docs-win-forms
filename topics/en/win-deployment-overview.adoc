...
The first step in deploying applications that include tools from the {ProductName} toolset is to determine what resources you will need to include as part of your deployment package. Click on the links to learn what resources each of the elements requires as part of deployment.
...
In addition to individual assembly files listed for each control/component, the deployment of any {ProductName} control/component also requires the re-distribution of the following files:
...
|{ProductName} support assembly for .NET
...
|{ProductName} Portable Core assembly for .NET.
