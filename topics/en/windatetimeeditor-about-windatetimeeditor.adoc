...
* Era Support -- Display the era in which the DateTime falls using the `g` or `gg` mask. The era displayed is chosen with respect to the control's culture, which can be set on `FormatProvider`.
