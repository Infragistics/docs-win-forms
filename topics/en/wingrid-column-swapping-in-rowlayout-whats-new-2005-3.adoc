...
Before {ProductName} 2005 Volume 3, the column swapping functionality only worked in non-row-layout mode. {ProductName} 2005 Volume 3 implements this functionality in Row-Layout mode. No new properties were needed since the existing AllowColumnSwapping property is sufficient. Also this doesn't change the default behavior since AllowColumnSwapping is resolved NotAllowed by default.
