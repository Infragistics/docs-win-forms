...
As part of the {ProductName} 2006 Volume 1 release, we are offering the Infragistics CAB Extensibility Kit™. This product is an add-on to the {ProductName} (CLR 2.0) product, extending the {ProductName} product to provide seamless interoperation with the CAB framework. This add-on gives development teams the power to develop sophisticated smart client applications using the controls and components in the {ProductName} toolset. The kit also allows development teams to provide rich, fine-grained appearances and behaviors to enterprise-class applications using predefined looks and feels, presets, and appearance settings.

The Infragistics CAB Extensibility Kit installs a Infragistics CompositeUI source-code solution and a quick start solution (ported from Microsoft) that utilizes {ProductName} controls/components -- the key items to get your CAB application seamlessly integrated with the {ProductName} (CLR 2.0) product.
