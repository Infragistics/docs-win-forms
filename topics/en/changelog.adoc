...
```csharp
// 50px gap between two adjacent ranges, independent of gauge width
...
Added `RangeBarSeries` to `{DataChartName}` so you can render low-to-high ranges as horizontal bars by combining a `CategoryYAxis` with a `NumericXAxis`. For more information, see link:datachart-category-range-bar-series.html[Range Bar Series].
...
|`41075`
|ToolbarsManager
|ArgumentOutOfRangeException occurs in UltraToolbarsManager in net10
