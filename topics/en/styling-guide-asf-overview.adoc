...
* *World-class support* -- AppStylist comes with its own help system designed from the ground up. Geared toward the graphic designer, the help contents also feature F1 context-sensitive help. The developer isn't left out either; the Developer's Guide in the main {ProductName} help also includes assistance specifically geared toward the developer, as well as includes the entire AppStylist help.