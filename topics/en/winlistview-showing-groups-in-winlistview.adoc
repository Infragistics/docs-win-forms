...
This topic will help you understand Groups in the WinListView control. You will use three separate {ProductName} controls in conjunction with each other to demonstrate how Groups function and what they look like in the different views of WinListView. You will also become familiar with the WinListView designer and how to perform tasks using it.
...
Create a  pick:[win-forms="link:{ApiPlatform}win.ultrawineditors{ApiVersion}~infragistics.win.ultrawineditors.ultratoggleeditorbase~checkedchanged_ev.html[CheckedChanged]"]  event handler for ultraCheckEditor1 and add the following code to it:
