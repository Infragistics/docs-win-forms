= WinListView Using the Notification Badge

This topic demonstrates the usage of the NotificationBadge properties of the link:winlistview.html[UltraListView].

toc::[]

== Overview

The Notification Badges allow you to notify your users about every important event that requires their attention. Almost all of the settings related to notification badges are exposed in the `NotificationBadgeSettings` property, which can be found directly on the `UltraListView`, on each `UltraListViewItem`, and on each `UltraListViewGroup`. The only property related to the NotificationBadges that isn’t part of the `NotificationBadgeSettings` class is the `NotificationBadgeText`, which can be found on each `UltraListViewItem` and `UltraListViewGroup`.
...
The `NotificationBadgeSettings` property exposes an `Appearance` property. The `NotificationBadgeSettings` property is exposed on the `UltraListView` and on the individual items and groups. By default the notification badges will use a red background, red borders and a white foreground with a bold font.
...
// Use Calibri font for all items
...
This property is not part of the NotificationBadgeSettings class, but a separate one, which is directly on the list item or group. It determines the text that is displayed by the NotificationBadge.
...
ultraListViewItem2.NotificationBadgeText = "New!";
----
